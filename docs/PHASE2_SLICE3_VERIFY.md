...
- Live (2026-09-04): gọi qua proxy Slice 3 và đối chiếu direct ai tool với cùng
  Basic Auth; status/content-type/body bytes khớp. Không có backup tạo đồng thời,
  nên không phát sinh delta cần nới assertion.
- Live pre/post: AutoCycle PID giữ nguyên và alive; hash `cycle_state.json` và
  `client_database.json` giữ nguyên; không có request write, không restart/stop
  AutoCycle, không đổi scheduler ownership.
