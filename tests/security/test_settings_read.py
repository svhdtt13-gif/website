...
class Stub(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/api/status":
            ...
            self._send(json.dumps({"auto_telegram": True, "unknown": {"nested": CANARIES[3]}}).encode())
        elif Stub.mode == "allowlisted_canary":
            payload = dict(Stub.valid); payload["tunnel_port"] = CANARIES[0]; self._send(json.dumps(payload).encode())
        elif Stub.mode == "port_low":
            payload = dict(Stub.valid); payload["tunnel_port"] = 0; self._send(json.dumps(payload).encode())
        elif Stub.mode == "port_high":
            payload = dict(Stub.valid); payload["tunnel_port"] = 65536; self._send(json.dumps(payload).encode())
            ...
            self._send(json.dumps({"error": "upstream failed", "telegram_bot_token": CANARIES[0], "nested": {"password": CANARIES[2]}}).encode(), 503)
...
def main():
    ...
    env = dict(os.environ)
    env.update(AI_TOOL_API_BASE=f"http://127.0.0.1:{STUB_PORT}", AI_TOOL_USER="tester", AI_TOOL_PASS="dummy", WEBAPP_PORT=str(PROXY_PORT))
    ...
    try:
        ...
        data = body_json(raw)
        expected = {"tunnel_port": 18080, "auto_restart_tunnel": True, "auto_telegram": False, "auto_open_browser": True}
        check("valid response is exact positive allowlist JSON", status == 200 and data == expected and set(data).issubset(PUBLIC_KEYS) and "application/json" in ctype, str(data))
        check("valid response has no secret canary", not any(x.encode() in raw for x in CANARIES), raw.decode(errors="replace"))
        check("settings path and Basic auth forwarded", len(Stub.hits) == 1 and Stub.hits[0][1] == "/api/settings" and Stub.hits[0][2].startswith("Basic "))
        ...
        for mode, label in (("allowlisted_canary", "allowlisted canary"), ("port_low", "port below range"), ("port_high", "port above range")):
            Stub.mode = mode
            status, raw, _ = call(proxy, "/up/api/settings")
            data = body_json(raw)
            check(label + " -> generic 502", status == 502 and isinstance(data, dict) and data == {"error": "settings upstream response unavailable"} and not any(x.encode() in raw for x in CANARIES), raw.decode(errors="replace"))

        for mode, label in (("malformed", "malformed JSON"), ("array", "non-object JSON")):
            Stub.mode = mode
            status, raw, _ = call(proxy, "/up/api/settings")
            data = body_json(raw)
            check(label + " 200 -> generic 502", status == 502 and isinstance(data, dict) and "error" in data and not any(x.encode() in raw for x in CANARIES), raw.decode(errors="replace"))

        for mode, expected_status in (("http_error", 503), ("unauthorized", 401)):
            Stub.mode = mode
            status, raw, _ = call(proxy, "/up/api/settings")
            check("upstream error status sanitized", status == expected_status and body_json(raw) == {"error": "settings upstream response unavailable"} and not any(x.encode() in raw for x in CANARIES), raw.decode(errors="replace"))
        ...
        check("settings write attempts cause zero upstream writes", not [h for h in Stub.hits if h[0] != "GET"])
        ...
        check("settings service has positive allowlist and no raw return", "_PUBLIC_FIELDS" in source and "api/settings" in source and "return ai_tool.get" not in source)

        stub.shutdown(); stub.server_close(); time.sleep(0.2)
        status, raw, _ = call(proxy, "/up/api/settings", timeout=10)
        check("unreachable -> generic 502", status == 502 and body_json(raw) == {"error": "settings upstream response unavailable"} and not any(x.encode() in raw for x in CANARIES), raw.decode(errors="replace"))
