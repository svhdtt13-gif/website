...
SAFE = {"tunnel_port": 18080, "auto_restart_tunnel": True,
        "auto_telegram": True, "auto_open_browser": False}
...
class Stub(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/api/status":
            ...
            self._send(json.dumps(SOURCE).encode())
        else:
...
def main():
    ...
    try:
        ...
        status, raw, ctype = call(proxy, "/up/api/settings", "POST", payload, headers)
        data = json_body(raw)
        posts = [h for h in Stub.hits if h["method"] == "POST"]
        check("valid partial update -> redacted success", status == 200
              and data == {"status": "OK", "settings": SAFE}
              and "application/json" in ctype
              and not any(x.encode() in raw for x in CANARIES), raw.decode(errors="replace"))
        check("valid update forwards canonical safe JSON and owned marker",
              len(posts) == 1 and posts[0]["path"] == "/api/settings"
              and json.loads(posts[0]["body"]) == {"tunnel_port": 18081, "auto_telegram": False}
        ...
        invalid_requests = [
            ...
            ("bad port type", b'{"tunnel_port":"TOKEN_CANARY"}'),
            ("bad port low", b'{"tunnel_port":0}'),
            ("bad port high", b'{"tunnel_port":65536}'),
            ("bad bool type", b'{"auto_telegram":{"secret":"NESTED_SECRET_CANARY"}}'),
        ]
        ...
        for mode, label in (("corrupt_port", "corrupt allowlisted port response"),
                            ...
                            ("non_object", "non-object response"),
        ...
        with open(os.path.join(BACKEND, "services", "settings.py"), encoding="utf-8") as f:
            source = f.read()
        check("GET and POST use shared projector/validator",
